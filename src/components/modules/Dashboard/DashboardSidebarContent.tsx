...
const DashboardSidebarContent = ({dashboardHome, navItems, userInfo} : dashboardSidebarContentProps) => {
  ...
  return (
    <div className="hidden md:flex h-full w-64 flex-col border-r bg-card overflow-hidden">
            ...
            <nav className="space-y-6 px-4 py-4">
                {
                    navItems.map((section, sectionId) => (
                        <div key={sectionId}>
                            {
                                section.title && (
                                    <h4 className="mb-3 px-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">{section.title}</h4>
                                )
                            }
                                    ...
                                    section.items.map((item, id) => {
                                        ...
                                        return (
                                            <Link href={item.href} key={id} className={
                                                cn("flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-all", isActive ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent hover:text-accent-foreground")
                                            }>
        ...
        {/* User Info At Bottom - Sticky Footer */}
